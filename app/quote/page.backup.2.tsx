          ...
          <div className="space-y-16 md:space-y-24">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-12 max-w-7xl relative z-10">
              ...
              <div className="flex flex-col items-end gap-3 w-full md:w-64">
                <span className="text-[11px] font-mono text-[var(--charcoal)]/80 tracking-[0.2em] uppercase">
                  {`0${currentStep} / 03`}
                </span>
                <div className="w-full h-[2px] bg-[var(--charcoal)]/10 relative overflow-hidden rounded-full">
                  <motion.div
                    initial={false}
                    animate={{ width: currentStep === 1 ? "33.33%" : currentStep === 2 ? "66.66%" : "100%" }}
                    transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
                    className="absolute top-0 left-0 h-full bg-[var(--charcoal)]/80"
                  />
                </div>
              </div>
            </div>
                  ...
                  <div className="pt-8 flex flex-col sm:flex-row gap-6 sm:gap-12 items-center sm:items-baseline justify-end w-full">
                    ...
                    >
                      <motion.button
                        ...
                        className={`premium-btn group py-5 md:py-6 px-8 md:px-16 w-full text-white font-bold uppercase tracking-[0.15em] md:tracking-[0.2em] text-[13px] md:text-[15px] font-mono shadow-2xl ${isSubmitting || !(formData.contactName.trim() && formData.phone.trim() && formData.jobTitle.trim()) ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
                        {isSubmitting ? (
